...
def main(args):
    ...
    # Replace tf.gfile with Python standard libraries for compatibility
    ...
    # Start session with TF2 compatibility mode
    tf.compat.v1.disable_eager_execution()
    sess = utils.create_session()

    # Load model using updated session syntax
    ...
    # Create patches for concept discovery
    cd.create_patches(param_dict={'n_segments': [15, 50, 80]})

    # Save target class images for concept discovery
    ...
    # Save discovered concept images (resized and original size)
...
def parse_arguments(argv):
    ...
    parser.add_argument('--source_dir', type=str,
                        help="Directory where the network's class image folders and random concept folders are saved.",
                        default='./ImageNet')
    ...
    parser.add_argument('--bottlenecks', type=str,
                        help='Names of the target layers of the network (comma-separated)', default='mixed4c')
    parser.add_argument('--num_random_exp', type=int,
                        help="Number of random experiments used for statistical testing, etc.", default=20)
    ...
    parser.add_argument('--min_imgs', type=int,
                        help="Minimum number of images in a discovered concept", default=40)
